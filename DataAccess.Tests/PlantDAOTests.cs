...
using NUnit.Framework;
    ...
    public class PlantDAOTests : DatabaseTestFixture
    {
        ...
        [Test]
        public async Task CreateAsync_InvalidPresetId_ShouldThrowException()
        {
            // Arrange
            ClearDatabase();
    
            // Assuming an invalid PlantPresetId for testing
            var plantCreationDto = new PlantCreationDTO
            {
                Location = "TestLocation",
                PlantPresetId = 999, 
            ...
            // Act & Assert
            Assert.ThrowsAsync<Exception>(async () => await _plantDao.CreateAsync(plantCreationDto));
        }

        [Test]
        public async Task GetAsync_InvalidPlantId_ShouldThrowException()
        {
            // Arrange
            ClearDatabase();
    
            // Assuming an invalid plant ID for testing
            var invalidPlantId = 1;

            // Act & Assert
            Assert.ThrowsAsync<Exception>(async () => await _plantDao.GetAsync(invalidPlantId));
        }

        [Test]
        public async Task RemoveAsync_InvalidPlantId_ShouldThrowException()
        {
            // Arrange
            ClearDatabase();
            ...
            // Act & Assert
            Assert.ThrowsAsync<Exception>(async () => await _plantDao.RemoveAsync(invalidPlantId));
        }
